class DatosPersonalesPage {
  ...
  async cargar(usuario) {
     ...
     * Fecha de nacimiento.
     *
     * El sitio utiliza un datepicker que puede borrar el valor
     * cuando pierde el foco. Por eso se mantiene el mecanismo
     * que ya funcionaba en tu automatización.
     */
    await this.page.evaluate((fecha) => {
      ...
      if (!elemento) {
        throw new Error(
          'No se encontró el campo de fecha de nacimiento.'
        );
      }
    ...
    await this.page.evaluate((isoDate) => {
      ...
      if (!elemento) {
        throw new Error(
          'No se encontró el campo fechaPago.'
        );
      }
  ...
  toIsoDate(ddmmyyyy) {
    ...
    if (partes.length !== 3) {
      throw new Error(
        `Fecha inválida: ${ddmmyyyy}. Se esperaba DD/MM/YYYY.`
      );
    }
